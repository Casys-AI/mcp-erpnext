...
Deno.test("shared contract fixtures are synthetic, canonical, and DT-hashable", async () => {
  const root = await Deno.makeTempDir({ prefix: "buy-contract-fixtures-" });
  try {
    ...
    const captureText = (await Deno.readTextFile(
      `${root}/accepted/buy-source-capture.wrapper.json`,
    )).trimEnd();
    ...
    const complete = JSON.parse((await Deno.readTextFile(
      `${root}/accepted/buy-recorded-result.complete.json`,
    )).trimEnd());
