/**
 * Moteur i18n — pur, sans dépendance React ni ext-apps.
 *
 * Ce fichier doit passer `deno check` et `deno test` sans import map npm.
 * Le hook React useT() vit dans i18n-hook.ts.
 *
 * Patron d'injection (même principe que format.ts / setLocaleSource) :
 *   host-locale.ts appelle setLangSource(() => locale) au démarrage.
...
function translate(
  ...
  const raw: string = Object.hasOwn(catalog, key)
    ? catalog[key]
