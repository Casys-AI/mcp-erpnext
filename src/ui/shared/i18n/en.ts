/**
 * Catalogue anglais — tronc commun.
 *
 * Clés stables en anglais, objets plats.  Les agents de viewer ajoutent leurs
 * propres clés (doclist.*, kanban.*, invoice.*, …) dans ce même fichier.
 * Interpolation : {name} → remplacé par t(key, { name: value }).
 *
 * Règle absolue : toute clé présente ici doit exister dans fr.ts, et
 * réciproquement.  Le test i18n_test.ts le vérifie.
 */

export const en: Record<string, string> = {
  // ── Commun ──────────────────────────────────────────────────────────────
  "common.loading": "Loading…",
  "common.refreshing": "Refreshing…",
  "common.refresh": "Refresh",
  "common.close": "Close",
  "common.cancel": "Cancel",
  "common.submit": "Submit",
  "common.back": "Back",
  "common.retry": "Retry",
  "common.no_data": "No data available",
  "common.no_assignee": "No assignee",

  "common.filter.placeholder": "filter",
  "common.filter.label": "Filter documents",

  "common.pagination.prev": "Previous page",
  "common.pagination.next": "Next page",

  "common.error.refresh_failed": "Refresh failed",
  "common.error.refresh_no_data": "Refresh returned no data",
  "common.error.parse_failed": "Failed to parse payload",
  "common.error.load_failed": "Failed to load",

  "common.progress.label": "Progress",
  "common.assignees.label": "Assignees",

  // ── Generic document surface ─────────────────────────────────────────────
  "document.header": "Document header",
  "document.docstatus": "docstatus {value}",
  "document.sections": "Document sections",
  "document.fields": "Fields",
  "document.attachments": "Attachments",
  "document.surface": "Document {name}",
  "document.sidebar": "Document actions and attachments",
  "document.empty_value": "—",
  "document.boolean.true": "Yes",
  "document.boolean.false": "No",
  "document.progress_value": "{value}%",
  "document.system_fields": "System fields",
  "document.table.no_columns": "No displayable columns",
  "document.json": "Raw document JSON",
  "document.action.refresh_pending":
    "Change committed — refresh is still pending",
  "document.purchase_invoice.total": "Invoice total",
  "document.purchase_invoice.calculated": "Calculated",
  "document.purchase_invoice.rounding": "Rounding",
  "document.purchase_invoice.details": "Rounding details",
  "document.purchase_invoice.disable_rounded_total": "Disable Rounded Total",
  "document.purchase_invoice.hint.draft":
    "To keep the calculated total, enable “{label}” on the draft invoice, then save and review its total.",
  "document.purchase_invoice.hint.submitted":
    "“{label}” applies before submission. Enable it on future invoices to keep the calculated total.",
  "document.purchase_invoice.aria.adds":
    "Rounding adds {amount} to the invoice total",
  "document.purchase_invoice.aria.reduces":
    "Rounding reduces the invoice total by {amount}",
  "document.purchase_invoice.aria.adjustment": "Rounding adjustment {signed}",
  "document.purchase_invoice.submit.unconfirmed":
    "Submission could not be confirmed. Inspect the same invoice before retrying.",
  "document.purchase_invoice.submit.transport":
    "Submit response was not received. Inspect the same invoice before retrying; submission is unconfirmed.",
  "document.attachments.add": "Add file",
  "document.attachments.refresh": "Refresh attachments",
  "document.attachments.loading": "Loading attachments…",
  "document.attachments.empty": "No attachments yet",
  "document.attachments.unavailable":
    "Attachments are not available in this host",
  "document.attachments.private": "Private",
  "document.attachments.public": "Public",
  "document.attachments.privacy": "Attachment privacy",
  "document.attachments.privacy_private":
    "Private attachment. Activate to make it public",
  "document.attachments.privacy_public":
    "Public attachment. Activate to make it private",
  "document.attachments.private_hint":
    "Only authenticated ERPNext users with access can retrieve this file",
  "document.attachments.public_hint":
    "ERPNext will expose this file from its public files area",
  "document.attachments.download": "Download {name}",
  "document.attachments.download_short": "Download",
  "document.attachments.download_in_progress": "Downloading attachment…",
  "document.attachments.preview": "Preview {name}",
  "document.attachments.preview_loading": "Preparing preview…",
  "document.attachments.preview_loading_name": "Preparing {name}…",
  "document.attachments.preview_pdf": "PDF preview of {name}",
  "document.attachments.preview_truncated":
    "Only the beginning of this text file is shown",
  "document.attachments.preview_unsupported":
    "This format is not displayed inside the viewer",
  "document.attachments.preview_download_fallback":
    "You can still download the original file",
  "document.attachments.context_add": "Add to context",
  "document.attachments.context_added": "In context",
  "document.attachments.context_update": "Update context",
  "document.attachments.context_sharing": "Adding…",
  "document.attachments.context_note": "Optional context note",
  "document.attachments.context_note_placeholder":
    "What should the model pay attention to?",
  "document.attachments.context_entire_file": "Entire attachment",
  "document.attachments.context_error":
    "The host could not update the model context",
  "document.attachments.reading": "Reading file…",
  "document.attachments.reading_progress": "Reading {value}%",
  "document.attachments.uploading": "Uploading…",
  "document.attachments.relisting": "Verifying…",
  "document.attachments.dismiss_error": "Dismiss attachment error",
  "document.attachments.error.list": "Could not load attachments",
  "document.attachments.error.upload": "Could not upload this file",
  "document.attachments.error.too_large":
    "This file exceeds the {limit} MB upload limit",
  "document.attachments.error.relist":
    "File added, but the attachment list could not be verified",
  "document.attachments.error.download": "Could not download this file",
  "document.attachments.error.preview": "Could not preview this file",
  "document.attachments.error.host_denied":
    "The host cancelled or refused the download",

  // ── Mise en page / marqueurs ─────────────────────────────────────────────
  "layout.live": "live",

  // ── Statuts (libellés d'interface autour du statut, pas les valeurs ERPNext) ─
  "status.unknown": "Unknown",

  // ── chart-viewer ──────────────────────────────────────────────────────────
  "chart.loading": "Loading chart…",
  "chart.empty.message":
    "No chart data — run an analytics tool to load a chart",
  "chart.pie.no_data": "No data for this chart",
  "chart.pie.all_zero": "All values are zero",
  "chart.donut.total_label": "Total",
  "chart.scatter.no_data": "No scatter data",
  "chart.treemap.no_data": "No treemap data",
  "chart.error.parse_failed": "Failed to parse chart data",
  "chart.keyboard.target": "{label} · {series} · {value}",
  "chart.keyboard.target_single": "{label} · {value}",
  "chart.keyboard.help":
    "Left and right change the point; up and down change the series; Space adds or removes it from context; Enter opens or closes available details.",
  "chart.keyboard.help_context":
    "Left and right change the point; up and down change the series; Space adds or removes it from context.",
  "chart.keyboard.help_detail":
    "Left and right change the point; up and down change the series; Enter opens or closes available details.",
  "chart.keyboard.help_navigation":
    "Left and right change the point; up and down change the series.",
  "chart.keyboard.control": "Explore chart: {target}. {help}",
  "chart.context.visible_label": "{title} · visible chart",
  "chart.context.select_visible":
    "Add the visible subset of {title} to active context",
  "chart.legend.aria": "Visible chart series",
  "chart.legend.hide": "Hide {series}",
  "chart.legend.show": "Show {series}",
  "chart.legend.last_visible": "{series} is the last visible series",
  // ── /chart-viewer ─────────────────────────────────────────────────────────

  // ── stock-viewer ──────────────────────────────────────────────────────────
  "stock.title": "Stock Balance",
  "stock.loading": "Loading stock…",
  "stock.no_data": "No stock data — run a stock query to see levels",
  "stock.filter.no_results": "No matching entries",
  "stock.error.tool_error": "Tool returned an error",
  "stock.col.item": "Item",
  "stock.col.qty": "Qty",
  "stock.col.actual": "Actual",
  "stock.col.reserved": "Reserved",
  "stock.col.projected": "Projected",
  "stock.col.rate": "Rate",
  "stock.col.value": "Value",
  "stock.footer.total_value": "Total stock value",
  "stock.detail.loading": "Loading item…",
  "stock.detail.label.name": "Name",
  "stock.detail.label.group": "Group",
  "stock.detail.label.uom": "Unit",
  "stock.detail.label.rate": "Base price",
  "stock.detail.recent_movements": "Recent movements",
  "stock.detail.action.chart": "Stock chart",
  "stock.detail.action.item": "Item sheet",
  "stock.detail.action.entries": "Stock entries",
  // ── /stock-viewer ─────────────────────────────────────────────────────────

  // ── kpi-viewer ────────────────────────────────────────────────────────────
  "kpi.skeleton.label": "loading",
  "kpi.empty.title": "No KPI data available",
  "kpi.empty.hint": "Run a KPI analytics tool to display metrics",
  "kpi.drilldown.aria_detail": "View {label} details",
  "kpi.drilldown.aria_trend": "View {label} trend",
  "kpi.sparkline.weeks": "{n} weeks",
  "kpi.sparkline.trend_label": "trend →",
  "kpi.delta.vs_label": "vs",
  "kpi.status.auto_refresh": "Auto-refresh on focus",
  "kpi.drilldown.title_detail": "Click to drill down",
  "kpi.drilldown.title_trend": "Click to see trend details",
  // ── /kpi-viewer ───────────────────────────────────────────────────────────

  // ── invoice-viewer ────────────────────────────────────────────────────────
  "invoice.loading": "Loading invoice…",
  "invoice.no_data": "No invoice data",
  "invoice.error.tool_error": "Tool returned an error",
  "invoice.error.parse_failed": "Failed to parse invoice payload",
  "invoice.error.refresh_failed": "Refresh failed",
  "invoice.error.action_failed": "Action failed",
  "invoice.action.submitted": "Submitted",
  "invoice.action.cancelled": "Cancelled",
  "invoice.preview.title": "Preview — no host",
  "invoice.header.outstanding": "outstanding",
  "invoice.header.due": "due {date}",
  "invoice.table.col.item": "item",
  "invoice.table.col.qty": "qty",
  "invoice.table.col.rate": "rate",
  "invoice.table.col.amount": "amount",
  "invoice.totals.subtotal": "subtotal",
  "invoice.totals.taxes": "taxes",
  "invoice.totals.grand_total": "grand total",
  "invoice.btn.related.title": "Open {label}",
  "invoice.btn.submit.title": "Submit invoice",
  "invoice.btn.submit.label": "Submit",
  "invoice.btn.cancel.title": "Cancel invoice",
  "invoice.btn.cancel.label": "Cancel",
  "invoice.lines.count": "{n} line{s}",
  "invoice.item.error.load_failed": "Failed to load item",
  "invoice.item.position": "line {n} of {total}",
  "invoice.item.close.aria_label": "Close item detail",
  "invoice.item.loading": "Loading item…",
  "invoice.item.uom": "unit",
  "invoice.item.warehouse": "warehouse",
  "invoice.item.vat": "vat",
  "invoice.item.available": "avail.",
  "invoice.item.btn.stock.title": "View item stock",
  "invoice.item.btn.stock.label": "Full stock view",
  "invoice.item.btn.details.title": "View item details",
  "invoice.item.btn.details.label": "Item details",
  "invoice.item.label.name": "Name",
  "invoice.item.label.group": "Group",
  "invoice.item.label.uom": "UOM",
  "invoice.item.label.std_rate": "Std Rate",
  "invoice.item.label.stock": "Stock",
  "invoice.party.type.customer": "Customer",
  "invoice.party.type.supplier": "Supplier",
  "invoice.status.paid": "Paid",
  "invoice.status.unpaid": "Unpaid",
  "invoice.header.date": "Date",
  "invoice.header.due_label": "Due",
  // ── /invoice-viewer ───────────────────────────────────────────────────────

  // ── funnel-viewer ─────────────────────────────────────────────────────────
  "funnel.loading": "Loading funnel…",
  "funnel.empty": "No funnel data — run the sales funnel tool",
  "funnel.empty.title": "No funnel data",
  "funnel.empty.hint": "Run the sales funnel tool to visualize your pipeline",
  "funnel.conversion.end_to_end": "{rate}% end to end",
  "funnel.conversion.overall": "Overall Conversion",
  "funnel.autorefresh": "Auto-refresh on focus",
  "funnel.stage.click_to_see": "Click to see {label}",
  "funnel.stage.aria_open": "Open {label}, {count}",
  "funnel.stage.aria_context":
    "Add or remove {label}, {count}, from active context",
  "funnel.stage.aria_ask": "Ask about {label}, {count}",
  "funnel.error.parse_failed": "Failed to parse funnel payload",
  "funnel.drill_down.leads": "Show all leads",
  "funnel.drill_down.opportunities": "Show all open opportunities",
  "funnel.drill_down.quotations": "Show all quotations",
  "funnel.drill_down.orders": "Show all sales orders",
  // ── /funnel-viewer ────────────────────────────────────────────────────────

  // ── doclist-viewer ────────────────────────────────────────────────────────
  "doclist.empty.message":
    "No document to display — query a DocType to see results",
  "doclist.error.tool_error": "Tool returned an error",
  "doclist.error.parse_payload": "Failed to parse document list",
  "doclist.error.load_details": "Failed to load details",
  "doclist.title.default": "Documents",
  "doclist.preview.no_refresh": "Preview — no host to refresh",
  "doclist.preview.no_host": "Preview — no host",
  "doclist.chips.all": "all {n}",
  "doclist.chips.filter_count": "{n} filter{s}",
  "doclist.chips.row_count": "{n} row{s}",
  "doclist.table.no_match": "No document matches",
  "doclist.table.header.id": "ref",
  "doclist.table.header.party": "party",
  "doclist.table.header.due": "due",
  "doclist.detail.action_failed": "Action failed",
  "doclist.detail.document_fallback": "Document",
  "doclist.detail.array_count": "{n} row{s}",
  "doclist.detail.full_detail": "Full detail",
  "doclist.detail.full_detail_message":
    "Show me the full detail of {doctype} {id}",
  "doclist.detail.action.submit_ok": "Submitted",
  "doclist.detail.action.submit_title": "Submit document",
  "doclist.detail.action.cancel_ok": "Cancelled",
  "doclist.detail.action.cancel_label": "Cancel document",
  "doclist.detail.back_to_list": "Back to list",
  "doclist.detail.inspector_label": "details",
  "doclist.detail.close_inspector": "Close details",
  "doclist.detail.back_list_label": "‹ list",
  // ── /doclist-viewer ───────────────────────────────────────────────────────
  // ── drill-down (partage d'une sélection) ──
  "drill.context": "Active selection in “{view}”: {label}{value}.",
  "drill.shared": "Selection shared — ask for details in the conversation",
  "drill.sent": "Sent to the conversation",
  "message.sending": "Sending…",
  "message.send_error": "Could not send to the conversation",
  "context.active.label": "Context",
  "context.active.items": "Active context selections",
  "context.active.select": "Add {label} to active context",
  "context.active.clear": "Remove {label} from active context",
  "context.active.clear_all": "Clear all",
  "context.active.error": "Context not updated",
  "context.active.pending": "Updating context…",
  "context.active.evicted": "8 max · removed {label}",
  "interaction.detail.open": "Open details for {label}",
  "interaction.detail.close": "Close details for {label}",
  "interaction.detail.open_hint": "Details · double-click",
  "interaction.detail.close_hint": "Close · double-click",
  "document.row.open_actions":
    "Click or Space to add or remove {label} from context · double-click or Enter to show its actions",
  "document.row.close_actions":
    "Click or Space to add or remove {label} from context · double-click or Enter to close its actions",
  "document.row.open_actions_only":
    "Double-click or press Enter to show actions for {label}",
  "document.row.close_actions_only":
    "Double-click or press Enter to close actions for {label}",
  "document.row.open_detail":
    "Click or Space to add or remove {label} from context · double-click or Enter to open its details",
  "document.row.close_detail":
    "Click or Space to add or remove {label} from context · double-click or Enter to close its details",
  "document.row.open_detail_only":
    "Double-click or press Enter to open details for {label}",
  "document.row.close_detail_only":
    "Double-click or press Enter to close details for {label}",
  "chart.tooltip.click_action_context":
    "click: add/remove context · 2× details",
  "chart.tooltip.click_action_context_only": "click: add/remove context",
  "chart.tooltip.click_action_fallback": "2× details",
  // ── pile de navigation (fil, niveaux, sauts) ──
  "nav.back": "Back",
  "nav.elided": "…{n}",
  "nav.elided_title": "Kept levels",
  "nav.kind.list": "list",
  "nav.kind.record": "record",
  "nav.kind.chart": "chart",
  "nav.loading": "call",
  "nav.ui.sort": "sort {key} {dir}",
  "nav.ui.page": "page {n}",
  "nav.ui.active": "{id} active",
  "nav.ui.filter": "filter “{q}”",
  "nav.goto": "go to",
  "nav.ask": "Ask a question",
  "nav.linked_to": "linked to {id}",
  "nav.of": "{n} of {total}",
  "nav.stale_values": "values as of {at}",
  "nav.refresh": "refresh",
  "nav.stale_title":
    "An action deeper in the stack changed what this level shows.",
  "nav.trail": "Navigation trail",
  "kanban.nav.edit_card": "edit {id}",
  "nav.bar_click": "click: one more level",
  "nav.unexpected_body": "This level received an unexpected body.",
  // ── confirmation des actions irréversibles ──
  "confirm.eyebrow": "Irreversible action",
  "invoice.confirm.cancel.detail":
    "The invoice will be set to Cancelled and its payments unlinked. ERPNext cannot undo this: the invoice would have to be amended.",
  "invoice.confirm.cancel.action": "Cancel the invoice",
  "invoice.confirm.submit.detail":
    "The invoice will be submitted and frozen: only a cancellation followed by an amendment can change it afterwards.",
  "invoice.confirm.submit.detail_with_amount":
    "The invoice will be submitted for {amount} and frozen: only a cancellation followed by an amendment can change it afterwards.",
  "invoice.confirm.submit.action": "Submit the invoice",
  "invoice.confirm.submit.action_with_amount": "Submit {amount}",
  "doclist.confirm.cancel.detail":
    "The document will be set to Cancelled and its links undone. ERPNext cannot undo this.",
  "doclist.confirm.cancel.action": "Cancel the document",
  "doclist.confirm.submit.detail":
    "The document will be submitted and frozen: only a cancellation followed by an amendment can change it afterwards.",
  "doclist.confirm.submit.detail_with_amount":
    "The document will be submitted for {amount} and frozen: only a cancellation followed by an amendment can change it afterwards.",
  "doclist.confirm.submit.action": "Submit the document",
  "doclist.confirm.submit.action_with_amount": "Submit {amount}",
  "invoice.confirm.submit": "Submit this invoice?",
  "invoice.confirm.cancel": "Cancel this invoice?",
  "doclist.confirm.submit": "Submit this document?",
  "doclist.confirm.cancel": "Cancel this document?",
  "doclist.hint.orders": "Orders",
  "doclist.hint.invoices": "Invoices",
  "doclist.hint.invoice": "Invoice",
  "doclist.hint.delivery": "Delivery",
  "doclist.hint.deliveries": "Delivery notes",
  "doclist.hint.payments": "Payments",
  "doclist.hint.stock": "Stock",
  "doclist.hint.po": "Purchase orders",
  "doclist.hint.receipt": "Receipt",
  "doclist.hint.attendance": "Attendance",
  "doclist.hint.leaves": "Leaves",
  "doclist.hint.tasks": "Tasks",
  "doclist.hint.timesheets": "Timesheets",
  "doclist.hint.opportunities": "Opportunities",
  "doclist.hint.customer": "Customer",
  "doclist.hint.supplier": "Supplier",
  "doclist.hint.lead": "Lead",
  "doclist.hint.item": "Item",
  "doclist.hint.movements": "Stock entries",
  "doclist.hint.warehouse": "Warehouse stock",
  "doclist.hint.sales_orders_month": "Sales orders this month",
  "doclist.hint.revenue_trend": "Revenue trend",
  "doclist.hint.unpaid_invoices": "Unpaid invoices",
  "doclist.hint.receivables_aging": "Receivables aging",
  "doclist.hint.order_breakdown": "Order breakdown",
  "doclist.hint.gross_profit_items": "Gross profit by item",
  "doclist.hint.profit_loss": "Profit and loss",
  "doclist.hint.overdue_invoices": "Overdue invoices",
  "doclist.hint.leads": "Leads",
  "doclist.hint.quotations": "Quotations",
  "doclist.hint.sales_orders": "Sales orders",
  // ── messages envoyés à la conversation (boutons explicites) ──
  "common.error.refresh_timeout": "Refresh timed out",
  "common.error.no_payload": "No text payload returned by tool call",
  "common.error.tool_failed": "Tool call failed",
  "common.error.no_proxy": "Host does not support proxied server tool calls",
  "kanban.error.user_list":
    "Could not read the user list returned by the server",
  "common.yes": "Yes",
  "common.no": "No",
  "kanban.error.move_timeout": "Move timed out, please retry.",
  "kanban.nav.view_list.message":
    "Show {doctype} {id} in a list limited to this document.",
  "kanban.nav.timesheets.message": "Show timesheets for task {id}",
  "kanban.nav.quotations.message": "Show quotations linked to opportunity {id}",
  "kanban.nav.tasks.message": "Show tasks related to issue {id}",
  "stock.nav.chart.message": "Show the stock chart for item {itemCode}",
  "stock.nav.details.message": "Show me the full record of item {itemCode}",
  "stock.nav.entries.message": "Show stock entries for item {itemCode}",
  "common.total": "total",
  "invoice.item.stock.message": "Show the stock of item {itemCode}",
  "invoice.item.details.message": "Show me the full record of item {itemCode}",

  // ── kanban-viewer ─────────────────────────────────────────────────────────
  "kanban.loading": "Loading board…",
  "kanban.no_data": "No kanban data available",
  "kanban.column.empty": "No cards in {label}",
  "kanban.card.milestone": "milestone",
  "kanban.card.aria_options": "Options for {title}",
  "kanban.card.due": "due {date}",
  "kanban.card.overdue": "{n} d overdue",
  "kanban.card.aria_move": "Move {title} to {label}",
  "kanban.nav.prev_column": "Previous column",
  "kanban.nav.next_column": "Next column",
  "kanban.live.moved": "Moved {title} to {label}",
  "kanban.live.moving": "Moving {title} to {label}",
  "kanban.live.queued": "{title} queued for {label}",
  "kanban.live.move_not_allowed": "Move to {label} is not allowed",
  "kanban.error.no_payload": "No kanban data received",
  "kanban.error.parse_failed": "Failed to parse kanban data",
  "kanban.error.connect_failed": "Failed to connect to MCP host",
  "kanban.error.detail_no_payload": "No detail data received",
  "kanban.error.detail_fetch_failed": "Failed to load card detail",
  "kanban.error.drag_drop": "Failed to process drag and drop",
  "kanban.error.move_failed": "Move failed",
  "kanban.modal.move_to": "Move to",
  "kanban.modal.save": "Save",
  "kanban.modal.saving": "Saving…",
  "kanban.modal.saved": "Saved",
  "kanban.modal.save_error": "Failed to save",
  "kanban.modal.discard.title": "Discard unsaved changes?",
  "kanban.modal.discard.detail":
    "This card has changes that have not been saved. Go back to keep editing, or discard them to continue.",
  "kanban.modal.discard.action": "Discard and close",
  "kanban.modal.discard.move_action": "Discard and move",
  "kanban.modal.discard.continue_action": "Discard and continue",
  "kanban.modal.section.general": "general",
  "kanban.modal.section.progress": "progress",
  "kanban.modal.section.assignees": "assignees",
  "kanban.section.dates": "dates",
  "kanban.section.time": "workload and time",
  "kanban.section.financial": "financial",
  "kanban.section.people": "people",
  "kanban.section.details": "other",
  "kanban.modal.nav.view_list": "Show this document in a list",
  "kanban.modal.nav.view_list.Task": "Show this task in a list",
  "kanban.modal.nav.view_list.Opportunity": "Show this opportunity in a list",
  "kanban.modal.nav.view_list.Issue": "Show this issue in a list",
  "kanban.modal.nav.request_list.Task": "Request this task in a list",
  "kanban.modal.nav.request_list.Opportunity":
    "Request this opportunity in a list",
  "kanban.modal.nav.request_list.Issue": "Request this issue in a list",
  "kanban.timesheets.loading": "Checking timesheets…",
  "kanban.timesheets.empty": "No timesheets",
  "kanban.timesheets.error": "Could not check timesheets",
  "kanban.timesheets.unavailable": "Timesheet availability not checked",
  "kanban.modal.nav.request_timesheets": "Request timesheets",
  "kanban.modal.nav.timesheets": "Timesheets",
  "kanban.modal.nav.quotations": "Quotations",
  "kanban.modal.nav.related_tasks": "Related tasks",
  "kanban.modal.bool.yes": "Yes",
  "kanban.modal.bool.no": "No",
  "kanban.modal.milestone.set_title": "Set as milestone",
  "kanban.modal.milestone.remove_title": "Milestone (click to remove)",
  "kanban.assignees.remove_aria": "Remove {email}",
  "kanban.assignees.select_label": "Assign to a user",
  "kanban.assignees.placeholder_error": "Users unavailable",
  "kanban.assignees.placeholder_select": "Assign to…",
  "kanban.assignees.assigning": "Assigning…",
  "kanban.assignees.assign_btn": "Assign",
  "kanban.assignees.error.load_users": "Failed to load users",
  "kanban.assignees.error.assign": "Failed to assign",
  "kanban.assignees.error.unassign": "Failed to remove assignee",
  "kanban.field.name": "ID",
  "kanban.field.subject": "subject",
  "kanban.field.status": "status",
  "kanban.field.priority": "priority",
  "kanban.field.project": "project",
  "kanban.field.progress": "progress",
  "kanban.field.description": "description",
  "kanban.field.exp_start_date": "start date",
  "kanban.field.exp_end_date": "due date",
  "kanban.field.expected_time": "estimated (h)",
  "kanban.field.actual_time": "actual time (h)",
  "kanban.field.is_milestone": "milestone",
  "kanban.field.task_weight": "weight",
  "kanban.field.total_costing_amount": "cost",
  "kanban.field.total_billing_amount": "billing",
  "kanban.field.start": "start",
  "kanban.field.duration": "duration",
  "kanban.field.title": "title",
  "kanban.field.opportunity_from": "source type",
  "kanban.field.party_name": "party",
  "kanban.field.opportunity_amount": "amount",
  "kanban.field.currency": "currency",
  "kanban.field.probability": "probability (%)",
  "kanban.field.opportunity_owner": "owner",
  "kanban.field.expected_closing": "expected closing",
  "kanban.field.transaction_date": "creation date",
  "kanban.field.contact_person": "contact",
  "kanban.field.source": "source",
  "kanban.field.customer": "customer",
  "kanban.field.raised_by": "created by",
  "kanban.field.resolution_by": "SLA deadline",
  "kanban.field.opening_date": "opening date",
  "kanban.field.resolution_date": "resolution",
  "kanban.field.first_responded_on": "first response",
  "kanban.select.priority.Low": "Low",
  "kanban.select.priority.Medium": "Medium",
  "kanban.select.priority.High": "High",
  "kanban.select.priority.Urgent": "Urgent",
  "kanban.select.opportunity_from.Lead": "Lead",
  "kanban.select.opportunity_from.Customer": "Customer",
  // ── /kanban-viewer ────────────────────────────────────────────────────────

  // ── buy-evidence-viewer ───────────────────────────────────────────────────
  "buy.title": "Buy configuration cost",
  "buy.field.coverage": "Coverage",
  "buy.field.quantity_basis": "Quantity basis",
  "buy.field.currency": "Currency",
  "buy.field.project": "Project",
  "buy.field.configuration_revision": "Configuration revision",
  "buy.field.observed_at": "Observed at",
  "buy.field.price_date": "Price date",
  "buy.field.covered_subtotal": "Covered subtotal",
  "buy.field.complete_total": "Complete total",
  "buy.field.description": "Description",
  "buy.table.selected_lines": "Selected lines",
  "buy.table.excluded_lines": "Unpriced / excluded lines",
  "buy.table.gaps": "Gaps",
  "buy.table.source_captures": "Source captures",
  "buy.col.item": "Item",
  "buy.col.source": "Source",
  "buy.col.unit_price": "Unit price",
  "buy.col.currency": "Currency",
  "buy.col.category": "Category",
  "buy.col.kind": "Kind",
  "buy.col.document_type": "Document type",
  "buy.col.modified": "Modified",
  "buy.col.site": "Site",
  "buy.col.fingerprint": "Fingerprint",
  "buy.col.row": "Row",
  "buy.col.description": "Description",
  "buy.col.quantity": "Quantity",
  "buy.col.unit": "Unit",
  "buy.col.amount": "Amount",
  "buy.col.price_date": "Price date",
  "buy.col.valid_from": "Valid from",
  "buy.col.valid_until": "Valid until",
  "buy.col.code": "Code",
  "buy.col.reason": "Reason",
  "buy.col.line": "Line",
  "buy.col.captured_at": "Captured at",
  // ── /buy-evidence-viewer ──────────────────────────────────────────────────
  // Kanban canonical labels and context toggle gestures.
  "kanban.status.open": "Open",
  "kanban.status.working": "Working",
  "kanban.status.pending_review": "Pending Review",
  "kanban.status.overdue": "Overdue",
  "kanban.status.completed": "Completed",
  "kanban.status.cancelled": "Cancelled",
  "kanban.status.replied": "Replied",
  "kanban.status.quotation": "Quotation",
  "kanban.status.converted": "Converted",
  "kanban.status.closed": "Closed",
  "kanban.status.lost": "Lost",
  "kanban.status.on_hold": "On Hold",
  "kanban.status.resolved": "Resolved",
  "kanban.transition.start_work": "Start work",
  "kanban.transition.request_review": "Request review",
  "kanban.transition.resume_work": "Resume work",
  "kanban.transition.approve": "Approve",
  "kanban.transition.reopen": "Reopen",
  "kanban.transition.reply": "Reply",
  "kanban.transition.send_quotation": "Send quotation",
  "kanban.transition.convert": "Convert",
  "kanban.transition.close": "Close",
  "kanban.transition.mark_lost": "Mark lost",
  "kanban.transition.resume_conversation": "Resume conversation",
  "kanban.transition.resume": "Resume",
  "kanban.transition.put_on_hold": "Put on hold",
  "kanban.transition.resolve": "Resolve",
  "kanban.metric.progress": "Progress",
  "kanban.metric.due": "Due",
  "kanban.metric.start": "Start",
  "kanban.metric.estimated": "Est.",
  "kanban.metric.actual": "Actual",
  "kanban.metric.amount": "Amount",
  "kanban.metric.probability": "Probability",
  "kanban.metric.closing": "Closing",
  "kanban.metric.created": "Created",
  "kanban.metric.raised_by": "Raised By",
  "kanban.metric.sla": "SLA",
  "kanban.metric.opened": "Opened",
  "kanban.metric.resolved": "Resolved",
  "kanban.badge.sla_breach": "SLA breach",
  "kanban.field.company": "company",
  "kanban.field.notes": "notes",
  "kanban.field.resolution_details": "resolution details",
  "context.active.toggle": "Add or remove {label} from active context",

  // Stable viewer labels and navigation preserve the existing UI.
  "stable.action.confirm": "Confirm?",
  "stable.brand.tagline": "ERP management",
  "stable.common.no_data": "No data available",
  "stable.common.waiting_tool": "Waiting for MCP tool result...",
  "stable.chart.empty":
    "No chart data — run an analytics tool or PML workflow that returns ChartData",
  "stable.chart.error.parse_failed": "Failed to parse chart payload",
  "stable.chart.pie.no_data": "No data for chart",
  "stable.chart.refreshing_button": "Refreshing",
  "stable.kpi.empty": "No KPI data — run an analytics KPI tool",
  "stable.kpi.error.parse_failed": "Failed to parse KPI payload",
  "stable.kpi.refreshing_button": "Refreshing",
  "stable.funnel.refreshing_button": "Refreshing",
  "stable.doclist.status.active": "Active",
  "stable.doclist.status.enabled": "Enabled",
  "stable.doclist.status.to_deliver_and_bill": "To Deliver and Bill",
  "stable.doclist.status.to_bill": "To Bill",
  "stable.doclist.status.to_deliver": "To Deliver",
  "stable.doclist.status.draft": "Draft",
  "stable.doclist.status.pending": "Pending",
  "stable.doclist.status.partly_paid": "Partly Paid",
  "stable.doclist.status.partly_delivered": "Partly Delivered",
  "stable.doclist.status.disabled": "Disabled",
  "stable.doclist.column.name": "name",
  "stable.doclist.column.type": "type",
  "stable.doclist.column.item_code": "item code",
  "stable.doclist.column.item_name": "item name",
  "stable.doclist.column.customer_name": "customer name",
  "stable.doclist.column.supplier": "supplier",
  "stable.doclist.column.supplier_name": "supplier name",
  "stable.doclist.column.employee_name": "employee name",
  "stable.doclist.column.outstanding_amount": "outstanding amount",
  "stable.doclist.column.posting_date": "posting date",
  "stable.doclist.column.transaction_date": "transaction date",
  "stable.doclist.column.delivery_date": "delivery date",
  "stable.doclist.column.workflow_state": "workflow state",
  "stable.doclist.column.docstatus": "docstatus",
  "stable.doclist.column.territory": "territory",
  "stable.doclist.column.customer_group": "customer group",
  "stable.doclist.column.item_group": "item group",
  "stable.doclist.column.supplier_group": "supplier group",
  "stable.doclist.column.warehouse": "warehouse",
  "stable.doclist.empty.title": "No documents to display",
  "stable.doclist.empty.help": "Query a DocType to see results in this table",
  "stable.doclist.records": "{n} of {total} records",
  "stable.doclist.search": "Search...",
  "stable.doclist.no_match": "No matching records",
  "stable.doclist.page": "Page {n} of {total}",
  "stable.doclist.pagination.first": "First",
  "stable.doclist.pagination.prev": "Prev",
  "stable.doclist.pagination.next": "Next",
  "stable.doclist.pagination.last": "Last",
  "stable.doclist.chips.all": "All",
  "stable.doclist.array_count": "{n} item{s}",
  "stable.stock.empty.title": "No stock data",
  "stable.stock.empty.help":
    "Run a stock balance query to see inventory levels",
  "stable.stock.entries": "{n} entries",
  "stable.stock.filtered_entries": "{n} entries (filtered from {total})",
  "stable.stock.filter": "Filter items / warehouses...",
  "stable.stock.error.parse_failed": "Failed to parse stock payload",
  "stable.stock.col.item_code": "Item Code",
  "stable.stock.col.warehouse": "Warehouse",
  "stable.stock.col.actual_qty": "Actual Qty",
  "stable.stock.detail.name": "Name",
  "stable.stock.detail.group": "Group",
  "stable.stock.detail.uom": "UOM",
  "stable.stock.detail.std_rate": "Std Rate",
  "stable.stock.detail.item_details": "Item details",
  "stable.invoice.error.name_required": "Invoice name is required.",
  "stable.invoice.doctype.purchase": "Purchase Invoice",
  "stable.invoice.doctype.sales": "Sales Invoice",
  "stable.invoice.navigation.payments":
    "Show payment entries for {doctype} {name}",
  "stable.invoice.navigation.customer_label": "Customer invoices",
  "stable.invoice.navigation.supplier_label": "Supplier invoices",
  "stable.invoice.navigation.customer":
    "Show all sales invoices for customer {party}",
  "stable.invoice.navigation.supplier":
    "Show all purchase invoices for supplier {party}",
  "stable.invoice.item.navigation.stock":
    "Show stock balance for item {itemCode}",
  "stable.invoice.item.navigation.details":
    "Show me the full details of Item {itemCode}",
  "stable.invoice.item.error.load_failed": "Failed to load item details",
  "stable.invoice.status.paid": "Paid",
  "stable.invoice.status.submitted": "Submitted",
  "stable.invoice.status.draft": "Draft",
  "stable.invoice.status.cancelled": "Cancelled",
  "stable.invoice.status.overdue": "Overdue",
  "stable.invoice.status.unpaid": "Unpaid",
  "stable.invoice.status.partly_paid": "Partly Paid",
  "stable.invoice.status.return": "Return",
  "stable.kanban.scroll.left": "Scroll left",
  "stable.kanban.scroll.right": "Scroll right",
  "stable.kanban.columns_aria": "Kanban columns",
  "stable.kanban.summary.cards": "{n} cards",
  "stable.kanban.summary.loaded": "{n}+ cards loaded",
  "stable.kanban.summary.move_tool": "move tool {name}",
  "stable.kanban.modal.aria_detail": "Detail: {title}",
  "stable.kanban.modal.close_detail": "Close detail",
  "stable.kanban.modal.discard": "Discard",
  "stable.kanban.modal.open_doclist": "Open in doclist",
  "stable.kanban.priority.change_title": "Click to change priority",
  "stable.kanban.assignees.assigned_to": "Assigned to",
  "stable.kanban.assignees.unassigned": "Unassigned",
  "stable.kanban.assignees.unassign": "Unassign {email}",
  "stable.kanban.assignees.loading": "Loading users…",
  "stable.kanban.nav.view_list.message":
    "Show me a list view of {doctype} {id}",
  "stable.doclist.hint.orders_customer.message":
    "Show sales orders for customer {id}",
  "stable.doclist.hint.invoices_customer.message":
    "Show sales invoices for customer {id}",
  "stable.doclist.hint.invoices_order.message":
    "Show invoices linked to sales order {id}",
  "stable.doclist.hint.delivery_order.message":
    "Show delivery notes for sales order {id}",
  "stable.doclist.hint.payments_invoice.message":
    "Show payment entries for invoice {id}",
  "stable.doclist.hint.stock_item.message": "Show stock balance for item {id}",
  "stable.doclist.hint.orders_item.message":
    "Show sales orders containing item {id}",
  "stable.doclist.hint.orders_supplier.message":
    "Show purchase orders for supplier {id}",
  "stable.doclist.hint.invoices_supplier.message":
    "Show purchase invoices for supplier {id}",
  "stable.doclist.hint.receipts_order.message":
    "Show purchase receipts for order {id}",
  "stable.doclist.hint.invoices_purchase_order.message":
    "Show purchase invoices for order {id}",
  "stable.doclist.hint.attendance_employee.message":
    "Show attendance for employee {id}",
  "stable.doclist.hint.leaves_employee.message":
    "Show leave applications for employee {id}",
  "stable.doclist.hint.tasks_project.message": "Show tasks for project {id}",
  "stable.doclist.hint.timesheets_project.message":
    "Show timesheets for project {id}",
  "stable.doclist.hint.timesheets_task.message":
    "Show timesheets for task {id}",
  "stable.doclist.hint.opportunities_lead.message":
    "Show opportunities for lead {id}",
};
